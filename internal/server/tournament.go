...
func (s *Server) handleCreateTournament(w http.ResponseWriter, r *http.Request) {
	...
	s.log.InfoContext(ctx, "tournament created",
		...
		"rated", created.Rated, "count_points", created.CountPoints)
...
func (s *Server) tournamentListView(ctx context.Context, again *uuid.UUID) (templates.TournamentListView, error) {
	...
	var (
		...
		withFinal bool
		// A fresh form counts, and repeating an evening repeats the answers
		// it was played under.
		rated       = true
		countPoints bool
	)
...
func (s *Server) tournamentView(ctx context.Context, id uuid.UUID, kiosk bool) (templates.TournamentView, error) {
	...
	if signedIn && tour.Open() && len(booked) == 0 {
		...
		// Carried, or the form would hand back the constructor's default and
		// a tournament somebody set to not count would quietly start
