...
// sideOr keeps the column's promise that a side name is never empty. A caller
// that says nothing means the default, and the check constraint would
// otherwise turn that into a failed insert rather than an "A".
func sideOr(name, fallback string) string {
	if name == "" {
		return fallback
	}
	return name
...
const tournamentColumns = `id, name, format, status, best_of, points_to_win, ` +
	`with_final, rated, count_points, side_a, side_b, ` +
...
func (r tournamentRepo) Create(ctx context.Context, t domain.Tournament) (domain.Tournament, error) {
	...
	created, err := scanTournament(r.q.QueryRow(ctx, insert,
		...
		t.WithFinal, t.Rated, t.CountPoints,
		sideOr(t.SideA, domain.DefaultSideA), sideOr(t.SideB, domain.DefaultSideB),
		t.CreatedBy))
	if err != nil {
...
func scanTournament(row pgx.Row) (domain.Tournament, error) {
	...
	err := row.Scan(&t.ID, &t.Name, &format, &status, &t.BestOf, &t.PointsToWin,
		&t.WithFinal, &t.Rated, &t.CountPoints, &t.SideA, &t.SideB,
