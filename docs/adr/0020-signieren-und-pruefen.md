...
Objekten. Die in #230 vermutete Plattform-Anfrage entfällt damit.
...
5. **Geprüft wird an zwei Stellen, und beide sind benannt:**
   - **Am Cluster** durch eine Kyverno-`ClusterPolicy` mit `verifyImages`
     (`policy/verify-image-signature.yaml`), begrenzt auf `schmetterpause` und
     `schmetterpause-pr-*`, mit `mutateDigest: true`. Das ist die stärkere
     Aussage: sie betrifft das Image, das der Kubelet gleich startet.
...
6. **Die Policy startet auf `Audit` und geht nach drei sauberen Releases auf
   `Enforce`** — die Haltung, die Trivy hier schon eingenommen hat. Der Tag,
   an dem sie aufhört, eine Messung zu sein, wird in `docs/supply-chain.md`
   festgehalten.
...
10. **Die Signatur-Identität steht an vier Stellen, und ein Test vergleicht
    ...
    Aussteller und denselben Unterzeichner — einmal als Regexp, einmal als
    Glob. `scripts/signing_test.sh` leitet das eine aus dem anderen ab und
    läuft in der Lint-Stufe. Eine der vier zu verbreitern ist der leise Weg,
    auf dem dieses Tor aufhört, eines zu sein.
...
### Warum `mutateDigest: true`

Ohne das prüft die Policy die Signatur an dem, worauf das Tag im Moment der
Admission zeigt, und der Kubelet darf danach etwas anderes ziehen. Mit dem
Umschreiben steht im Pod der Digest, der geprüft wurde. `verifyDigest` ist
ohnehin standardmäßig an und das Deployment nennt ein Tag — die beiden
Einstellungen gehören zusammen.
...
- **Positiv:** `mutateDigest` schließt das Fenster zwischen „Tag geprüft" und
  „Image gezogen" — dieselbe Klasse von Fehler wie #177 und #179, nur zur
  Laufzeit.
...
- **Einschränkung: nur unser Image.** Die Regel greift für
  `ghcr.io/stuttgart-things/schmetterpause:*`. Postgres daneben im selben
  Namespace ist nicht geprüft und wird auch nicht als geprüft ausgegeben.
...
- **Positiv:** Die Lint-Stufe merkt, wenn die vier Kopien der
  Signatur-Identität auseinanderlaufen. Ohne das wäre ein verbreiteter Glob in
  der Policy eine grüne Pipeline und eine Prüfung, die nichts mehr aussagt —
  dieselbe Klasse wie #177 und #179, nur an der Stelle, die das verhindern
  soll.
- **Kosten:** zwei Jobs mehr pro Lauf, ein Trivy-Pull mehr, und zwei weitere
  gepinnte Werkzeugversionen in `ci.yml`, die Renovate bewegen muss.
- **Nötig von Hand:** die `ClusterPolicy` anwenden. Sie liegt nicht im
  Kustomize-Base, weil sie clusterweit ist und jeder Preview-Namespace sie
  sonst mit installieren und wegräumen würde (`policy/README.md`) — dieselbe
  Trennung, die ADR-0019 für die Backup-Konfiguration zieht.
...
2. **Wer die `ClusterPolicy` reconciled.** Heute von Hand. Sie in den
   argocd-Katalog zu legen, wäre der nächste Schritt — und die Frage, wem sie
   dann gehört.
